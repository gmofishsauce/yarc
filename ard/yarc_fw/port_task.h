...
namespace PortPrivate {
  ...
  void callWhenAnyReset() {
    ...
    // // Set the microcode for a write from sysdata to R3
    ...
    // R3 should now contain 0x7700. This can be verified with the scope
    // because R3 is the default on the source1 bus (when K is 0xFFFFFFFF).

    // Now write flags value, carry flag set and no others, to 0x7700.
    byte flagval = 0x05; // carry and negative set
    WriteMem8(0x7700, &flagval, 1);

    // Next write a minimal instruction to the bottom of microcode memory.
    // Two cycles: the instruction to clock (R3) to F and then 0xFFFFFFFF.
    // Here is the instruction to clock (R3) to F:
    // WriteByteToK(3, 0xff); // src1=R3 src2=-1 dst=?R3
    // WriteByteToK(2, 0xfe); // alu_op=alu_0x0F alu_ctl=alu_none alu_load_hold=no alu_load_flgs=yes
    // WriteByteToK(1, 0x9f); // sysdata_src=mem reg_in_mux=sysdata stack_up_clk=no stack_dn_clk=no psp_rsp=psp dst_wr_en=no write
    // WriteByteToK(0, 0xff); // rw=read m16_en=8-bit ir_clk=no load rsw_ir_uc=RCW from UC carry_en=Cin=Cflag ld_flags_mux=sysdata
    byte slice[2];
    slice[0] = 0xFF; slice[1] = 0xFF;
    writeBytesToSlice(0x80, 3, slice, 2);

    slice[0] = 0xFE; slice[1] = 0xFF;
    writeBytesToSlice(0x80, 2, slice, 2);

    slice[0] = 0x9F; slice[1] = 0xFF;
    writeBytesToSlice(0x80, 1, slice, 2);

    slice[0] = 0xFF; slice[1] = 0xFF;
    writeBytesToSlice(0x80, 0, slice, 2);

    // Verify
  #define FF ((byte)0xFF) // argh

    readBytesFromSlice(0x80, 3, slice, 2);
    if (slice[0] != FF || slice[1] != FF) {
      SetDisplay(0x81); for(;;) {}
    }
    readBytesFromSlice(0x80, 2, slice, 2);
    if (slice[0] != (byte)0xFE || slice[1] != FF) {
      SetDisplay(0xC1); for(;;) {}
    }
    readBytesFromSlice(0x80, 1, slice, 2);
    if (slice[0] != (byte)0x9F || slice[1] != FF) {
      SetDisplay(0xE1); for(;;) {}
    }
    readBytesFromSlice(0x80, 0, slice, 2);
    if (slice[0] != FF || slice[1] != FF) {
      SetDisplay(0xF1); for(;;) {}
    }

    // OK, enable the YARC and issue a clock
    MakeSafe();
    SetDisplay(0xAA);
    for(;;) {
      WriteIR(0x80, 0x00);
    ...
    //   SetDisplay(acrValue << 4);
    //   SetACR(acrValue);
    //   while (millis() < stamp + 60000L) {
    //     SingleClock();        
    //   }
    //   acrValue++;
    //   if (acrValue > 15) {
